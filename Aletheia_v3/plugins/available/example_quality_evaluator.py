...
from Aletheia_v3.core.domain import (
    get_quality as default_get_quality,  # Import default for comparison or fallback
)
...
class SimpleBonusQualityEvaluator(QualityEvaluatorPlugin):
    ...
    def initialize(self, config=None):
        ...
        if config:
            self._bonus_amount = config.get(
                self.CONFIG_KEY_BONUS_AMOUNT, self._bonus_amount
            )
            self._use_default_base = config.get(
                self.CONFIG_KEY_USE_DEFAULT_BASE, self._use_default_base
            )
        print(
            f"'{self.name}' configured: Bonus Amount = {self._bonus_amount}, Use Default Base = {self._use_default_base}"
        )
    ...
    def evaluate_quality(self, a: int, b: int) -> float:
        ...
        if self._use_default_base:
            ...
            plugin_modified_quality = (
                base_quality + self._bonus_amount
                if base_quality > 0
                else base_quality
            )
            ...
            plugin_modified_quality = (
                b / a
            ) * 0.001  # Arbitrary alternate logic
