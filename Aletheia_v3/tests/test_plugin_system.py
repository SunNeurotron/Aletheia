...
from Aletheia_v3.plugins.plugin_interfaces import (
    AletheiaPluginBase, QualityEvaluatorPlugin, SearchStrategyPlugin,
    DataPostprocessorPlugin, ParameterSpaceModifierPlugin
)
...
DUMMY_PLUGIN_BASE_DIR = os.path.join(os.path.dirname(__file__), "test_plugins_temp")
...
def create_dummy_plugin_file(plugin_dir, filename="dummy_plugin1.py", content=""):
...
class DummyQuality(QualityEvaluatorPlugin):
    @property
    def name(self): return "DummyTestQuality"
    @property
    def version(self): return "0.1"
    def evaluate_quality(self, a, b): return float(a + b)
    def initialize(self, config=None): pass # Simpler init for test
    def terminate(self): pass

class DummySearch(SearchStrategyPlugin):
    @property
    def name(self): return "DummyTestSearch"
    @property
    def version(self): return "0.1"
    def suggest_candidates(self, history_X=None, history_y=None, n_suggestions=1, search_space_dimensions=None, optimizer_state=None): return [[0.1, 0.2]]
...
"""
        create_dummy_plugin_file(temporary_plugin_dir, "my_plugin_module.py", plugin_content)
        ...
        def mock_import_module(name, package=None):
            if name == ".available_for_test.my_plugin_module": # Name manager tries to import
        ...
        with patch('importlib.import_module', side_effect=mock_import_module):
            ...
            plugin_manager.discover_plugins(plugin_dirs=[temporary_plugin_dir]) # Uses patched DEFAULT_PLUGIN_DIRS

        expected_plugin_id = "my_plugin_module.MyTestPlugin"
        assert expected_plugin_id in plugin_manager.list_available_plugin_classes()
    ...
    def test_discover_module_with_no_plugin_class(self, temporary_plugin_dir):
        create_dummy_plugin_file(temporary_plugin_dir, "not_a_plugin.py", "class NotAPlugin: pass")
        plugin_manager.discover_plugins(plugin_dirs=[temporary_plugin_dir])
        assert "not_a_plugin.NotAPlugin" not in plugin_manager.list_available_plugin_classes()

    def test_discover_module_with_abstract_plugin_class(self, temporary_plugin_dir):
        content = """
...
"""
        create_dummy_plugin_file(temporary_plugin_dir, "abstract_plugin.py", content)
        plugin_manager.discover_plugins(plugin_dirs=[temporary_plugin_dir])
        ...
        assert "abstract_plugin.MyAbstractPlugin" not in plugin_manager.list_available_plugin_classes()


    def test_import_error_in_plugin_module(self, temporary_plugin_dir, capsys):
        create_dummy_plugin_file(temporary_plugin_dir, "bad_import_plugin.py", "import non_existent_module")
        plugin_manager.discover_plugins(plugin_dirs=[temporary_plugin_dir])
        captured = capsys.readouterr()
        assert "Error importing plugin module bad_import_plugin" in captured.out
        assert len(plugin_manager.list_available_plugin_classes()) == 0
...
class TestPluginActivationAndLifecycle:
    ...
    def setup_dummy_plugin_classes(self, temporary_plugin_dir):
        ...
        plugin_manager._PLUGIN_CLASS_REGISTRY["dummy_module.DummyQuality"] = DummyQuality
        plugin_manager._PLUGIN_CLASS_REGISTRY["dummy_module.DummySearch"] = DummySearch
        yield
        plugin_manager._PLUGIN_CLASS_REGISTRY.clear()


    def test_get_activate_new_plugin_instance(self, setup_dummy_plugin_classes):
        instance = plugin_manager.get_active_plugin_instance("dummy_module.DummyQuality", config={"test_setting": 123})
        ...
        assert "dummy_module.DummyQuality" in plugin_manager.list_active_plugin_instances()

    def test_get_existing_active_instance(self, setup_dummy_plugin_classes):
        instance1 = plugin_manager.get_active_plugin_instance("dummy_module.DummyQuality")
        instance2 = plugin_manager.get_active_plugin_instance("dummy_module.DummyQuality")
        assert instance1 is instance2 # Should return same instance

    def test_force_reload_instance(self, setup_dummy_plugin_classes):
        instance1 = plugin_manager.get_active_plugin_instance("dummy_module.DummyQuality")
        instance2 = plugin_manager.get_active_plugin_instance("dummy_module.DummyQuality", force_reload=True)
        assert instance1 is not instance2 # Should be a new instance

    def test_get_non_existent_plugin_class(self):
        instance = plugin_manager.get_active_plugin_instance("non_existent_plugin.NoClass")
        assert instance is None

    def test_plugin_initialization_error(self, setup_dummy_plugin_classes, capsys):
        # Mock initialize to raise an error
        with patch.object(DummyQuality, 'initialize', side_effect=Exception("Init failed")):
            instance = plugin_manager.get_active_plugin_instance("dummy_module.DummyQuality")
            assert instance is None
            captured = capsys.readouterr()
            assert "Error instantiating or initializing plugin dummy_module.DummyQuality" in captured.out

    def test_shutdown_all_plugins(self, setup_dummy_plugin_classes):
        # Activate a couple of plugins
        dq_instance = plugin_manager.get_active_plugin_instance("dummy_module.DummyQuality")
        ds_instance = plugin_manager.get_active_plugin_instance("dummy_module.DummySearch")
