    ...
    with st.form("search_form"):
        ...
        if submitted:
            ...
            try:
                ...
                response = requests.post(f"{API_BASE_URL}/searches", json=payload, timeout=10)
                if response.status_code == 202: # HTTP 202 Accepted
                    ...
                    if job_id:
                        ...
                        st.success(f"Job '{job_id}' submitted successfully! Status: {job_info.get('status')}.")
                    else:
                        st.error("Submission successful, but no job ID received in response.")
                else:
                    st.error(f"API Error ({response.status_code}): {response.text}")
            except requests.exceptions.RequestException as e:
                st.error(f"API Connection Error: {e}. Is the API service running at {API_BASE_URL}?")
...
if not st.session_state.submitted_jobs:
    st.info("No jobs submitted yet. Use the sidebar to start a new discovery job.")
else:
    ...
    for job_id in st.session_state.submitted_jobs:
        try:
            response = requests.get(f"{API_BASE_URL}/searches/{job_id}", timeout=10)
            if response.status_code == 200:
                ...
                created_at_str = datetime.fromisoformat(created_at_raw).strftime('%Y-%m-%d %H:%M:%S UTC') if created_at_raw else "N/A"

                # Determine expander default state: expand processing or completed jobs
                expanded_default = status in ["PROCESSING", "COMPLETED"]

                with st.expander(f"Job ID: {job_id} | Status: {status} | Created: {created_at_str}", expanded=expanded_default):
                    ...
                    if status == "COMPLETED":
                        ...
                        if hits:
                            ...
                            df_display = df_hits[['a', 'b', 'c', 'quality']].copy()
                            df_display.rename(columns={'quality': 'Quality (q)'}, inplace=True)
                            st.dataframe(df_display)

                            # Plotting results
                            # Using log10(c) for x-axis can spread out points if c varies a lot.
                            # Ensure 'c' is positive before log.
                            valid_hits_for_plot = [h for h in hits if h.get('c', 0) > 0]
                            if valid_hits_for_plot:
                                log_c_values = [math.log10(h['c']) for h in valid_hits_for_plot]
                                qualities = [h['quality'] for h in valid_hits_for_plot]
                                hover_texts = [
                                ...
                                fig = go.Figure(data=[go.Scatter(
                                    ...
                                    y=qualities,
                                    mode='markers',
                                    marker=dict(size=10, color=qualities, colorscale='Viridis', showscale=True, colorbar=dict(title="Quality (q)")),
                                    text=hover_texts,
                                    hoverinfo='text'
                                )])
                                fig.update_layout(
                        ...
                        st.info("Job is pending and will be picked up by a worker soon.")
                    elif status == "FAILED" or "ERROR" in status:
                        st.error(f"Job encountered an error. Status: {status}. Check API logs or MLflow for details.")

            elif response.status_code == 404:
                st.error(f"Job ID '{job_id}' not found in the system. It might have been an invalid ID or an issue occurred.")
                # Optionally, remove invalid ID from session state
                # st.session_state.submitted_jobs.remove(job_id)
            else:
                st.warning(f"Could not retrieve status for job {job_id}. API responded with {response.status_code}: {response.text}")

        except requests.exceptions.RequestException as e:
            st.error(f"Failed to connect to API for job {job_id}: {e}")
        except Exception as e: # Catch any other unexpected errors during rendering
            st.error(f"An unexpected error occurred while displaying job {job_id}: {e}")

# --- Footer ---
st.markdown("---")
st.markdown(f"<p align='center'>Aletheia Platform v3.0 (MDU Edition) | Current API: {API_BASE_URL}</p>", unsafe_allow_html=True)
