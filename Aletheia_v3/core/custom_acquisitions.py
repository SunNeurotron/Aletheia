...
SMALL_PRIMES_FOR_HEURISTIC = [2, 3, 5, 7, 11, 13] # Tunable list of small primes
MAX_POWER_FOR_HEURISTIC = 40  # Max power to check (e.g., up to prime^40), tunable
...
def _get_prime_power_series(prime: int, value_upper_bound: int) -> list[int]:
    ...
    limit_exponent = 0
    if value_upper_bound > 1 and prime > 1: # Avoid log(1) or log_anything(1) if value_upper_bound is 1
        try:
            limit_exponent = int(math.log(value_upper_bound, prime)) + 2 # +2 for some margin
        except ValueError: # math domain error if value_upper_bound or prime is invalid for log
            return []
    ...
    for _ in range(1, effective_max_exponent + 1):
        if current_power > value_upper_bound * 1.5: # Check slightly beyond to catch near misses, but not too far
        ...
        if value_upper_bound / prime < current_power and prime > 1: # Avoid overflow for current_power * prime
             break
        current_power *= prime
...
def get_structural_bonus(value: int,
                        bonus_scale_factor: float = 0.05,
                        proximity_penalty_factor: float = 5.0,
                        exact_match_multiplier: float = 2.0) -> float:
    """
    ...
    for p in SMALL_PRIMES_FOR_HEURISTIC:
        ...
        if p > value and not (p == value): # if p==value, it's p^1
             if value*0.5 > p : # if p is much larger than value, skip. Heuristic.
                continue
        ...
        for pv_idx, pv in enumerate(power_series):
            ...
            relative_distance = distance / pv if pv > 0 else float('inf') # Should not be pv=0
    ...
    if is_exact_power:
        ...
        final_bonus = bonus_scale_factor * math.exp(-proximity_penalty_factor * min_relative_distance)
        # Threshold small bonuses to avoid noise
        if final_bonus < (bonus_scale_factor * 0.01): # e.g. less than 1% of base bonus
            final_bonus = 0.0
...
if __name__ == '__main__':
    ...
    print(f"Bonus for 32 (2^5): {get_structural_bonus(32)}") # Expect higher bonus
    print(f"Bonus for 27 (3^3): {get_structural_bonus(27)}") # Expect higher bonus
    print(f"Bonus for 25 (5^2): {get_structural_bonus(25)}") # Expect higher bonus
    ...
    print(f"Bonus for 100 (10^2, not prime power): {get_structural_bonus(100)}") # Will be low unless 100 is close to e.g. 3^4=81 or 5^3=125
                                                                            # Or if we extend SMALL_PRIMES_FOR_HEURISTIC to include 10 (not prime)
                                                                            # Or if code was changed to look for N*p^k etc.
                                                                            # Currently, only p^k.
    print(f"Bonus for 999 (near 1000=10^3 or other powers): {get_structural_bonus(999)}")
    print(f"Bonus for a prime like 17: {get_structural_bonus(17)}") # Bonus for being 17^1
