...
groups_strategy = st.lists(
    VALID_FLOAT, min_size=MIN_SAMPLES, max_size=MAX_SAMPLES
)

# Strategy for alpha value (significance level)
alpha_strategy = st.floats(
...
def test_perform_t_test_analysis_properties(
    ...
    try:
        ...
        if (
            any(len(set(g)) == 1 for g in [group_a, group_b])
            and "Shapiro-Wilk" in str(e).lower()
        ):
            pytest.skip(
                f"Skipping due to constant data in a group for Shapiro-Wilk: {e}"
            )
        ...
        if (
            "variance is zero" in str(e).lower()
            or "Degrees of freedom must be positive" in str(e).lower()
        ):
            pytest.skip(
                f"Skipping due to problematic variance or df for t-test: {e}"
            )
    ...
    )  # Should not be NaN/Inf for valid inputs
    ...
    # Degrees of freedom
    assert (
        result.degrees_freedom >= 1
    ), "Degrees of freedom should be >= 1"  # Welch-Satterthwaite df >= min(n1-1, n2-1)
    ...
    if not (
        np.isinf(result.confidence_interval_95[0])
        or np.isinf(result.confidence_interval_95[1])
        or np.isnan(result.confidence_interval_95[0])
        or np.isnan(result.confidence_interval_95[1])
    ):
        assert (
    ...
    # Normality p-values
    assert (
        0.0 <= result.normality_p_value_group_a <= 1.0
    ), "Normality p-value for A out of [0,1] range"
    assert (
        0.0 <= result.normality_p_value_group_b <= 1.0
    ), "Normality p-value for B out of [0,1] range"
    ...
    if (
        result.normality_p_value_group_a >= alpha
        and result.normality_p_value_group_b >= alpha
    ):
        assert (
            result.comment == "Both groups appear to be normally distributed."
            or result.comment == "Analysis complete."
        )  # if logic changes
...
def test_t_test_identical_groups_high_p_value(
    service: StatsService, group_a: List[float], alpha: float
):
    """
...
def test_t_test_shifted_groups(
    service: StatsService, group_a: List[float], shift: float, alpha: float
):
    """
    ...
    if len(set(group_a)) == 1 and abs(shift) > 1e-3:  # Constant group, shifted
        ...
        if (
            shift > 0
        ):  # group_b mean > group_a mean, so mean_a - mean_b is negative
            assert result.statistic < 0
        else:  # shift < 0, group_b mean < group_a mean, so mean_a - mean_b is positive
            assert result.statistic > 0
        return  # End test for this specific case
    elif (
        len(set(group_a)) == 1 and abs(shift) < 1e-3
    ):  # Constant group, not really shifted
    ...
    if abs(shift) < 1e-3:  # Effectively identical groups
        assert result.p_value > 0.1 or np.isclose(
            result.p_value, 1.0
        )  # Should be high
        assert np.isclose(result.statistic, 0.0, atol=1e-6) or np.isnan(
            result.statistic
        )
    else:
        ...
        if shift > 0:  # mean_a - mean_b is negative
            assert result.statistic < 0 or np.isnan(
                result.statistic
        ...
        if (
            std_dev_a > 1e-6 and abs(shift) / std_dev_a > 3
        ):  # Heuristic: shift is >3 std devs
...
def test_t_test_symmetry_statistic_pvalue(
    service: StatsService,
    group_a: List[float],
    group_b: List[float],
    alpha: float,
    ...
    if (
        len(set(group_a)) == 1
        and len(set(group_b)) == 1
        and np.mean(group_a) == np.mean(group_b)
    ) or (
        len(set(group_a)) == 1
        and len(set(group_b)) == 1
        and np.mean(group_a) != np.mean(group_b)
        ...
        pytest.skip(
            "Skipping symmetry test for constant groups as t-stat can be NaN or Inf."
        )

    try:
        result_ab = service.perform_t_test_analysis(
            group_a, group_b, alpha=alpha
        )
        result_ba = service.perform_t_test_analysis(
            group_b, group_a, alpha=alpha
        )
    except ValueError as e:
        if "Shapiro-Wilk" in str(e).lower() and (
            len(set(group_a)) == 1 or len(set(group_b)) == 1
        ):
            pytest.skip(
                f"Skipping due to constant data in a group for Shapiro-Wilk: {e}"
            )
    ...
    assert result_ab.p_value == pytest.approx(
        result_ba.p_value
    ...
    if not (np.isnan(result_ab.statistic) and np.isnan(result_ba.statistic)):
        assert result_ab.statistic == pytest.approx(
            -result_ba.statistic
        ), "T-statistics should be opposite for swapped groups"

    # Other fields should also be consistent
    assert result_ab.degrees_freedom == pytest.approx(
        result_ba.degrees_freedom
    )
    # CI for diff(A,B) is (L,U), CI for diff(B,A) is (-U, -L)
    assert result_ab.confidence_interval_95[0] == pytest.approx(
        -result_ba.confidence_interval_95[1]
    )
    assert result_ab.confidence_interval_95[1] == pytest.approx(
        -result_ba.confidence_interval_95[0]
    )

    assert result_ab.is_significant_05 == result_ba.is_significant_05
    assert result_ab.normality_p_value_group_a == pytest.approx(
        result_ba.normality_p_value_group_b
    )  # A in first is B in second
    assert result_ab.normality_p_value_group_b == pytest.approx(
        result_ba.normality_p_value_group_a
    )  # B in first is A in secondTool output for `create_file_with_block`:
