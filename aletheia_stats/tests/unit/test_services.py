...
def group_non_normal_D() -> list[float]:
    ...
    # np.random.seed(202)
    return list(
        np.random.exponential(scale=1.0, size=30) * 5
    )  # Scale to have similar magnitude
...
def test_perform_t_test_analysis_normal_groups_different_means(
    ...
    group_normal_A: list[float],
    group_normal_B: list[float],
):
    """
    Test t-test with two normally distributed groups that likely have different means.
    Expect p-value to be small (significant).
    """
    result = service.perform_t_test_analysis(
        group_normal_A, group_normal_B, alpha=ALPHA
    )
    ...
    assert (
        isinstance(result.degrees_freedom, float)
        and result.degrees_freedom > 0
    )
...
def test_perform_t_test_analysis_normal_groups_similar_means(
    service: StatsService,
    group_normal_A: list[float],
    group_normal_C_identical_to_A: list[float],
):
    """
    ...
    result = service.perform_t_test_analysis(
        group_normal_A, group_B_similar, alpha=ALPHA
    )  # Using group_normal_C_identical_to_A

    assert isinstance(result, TTestResult)
    assert (
        result.p_value >= ALPHA
    )  # Expect non-significance (p-value can be close to 1.0)
...
def test_perform_t_test_analysis_one_group_non_normal(
    ...
    """
    result = service.perform_t_test_analysis(
        group_normal_A, group_non_normal_D, alpha=ALPHA
    )
...
def test_perform_t_test_analysis_both_groups_non_normal(
    ...
    result = service.perform_t_test_analysis(
        group_non_normal_D, group_non_normal_E, alpha=ALPHA
    )
...
def test_perform_t_test_analysis_insufficient_data_for_normality(
    ...
    """
    with pytest.raises(
        ValueError, match="Each group must contain at least three observations"
    ):
        service.perform_t_test_analysis(
            group_small_E, group_normal_A, alpha=ALPHA
        )
    ...
    with pytest.raises(
        ...
        service.perform_t_test_analysis(
            group_normal_A, group_small_E, alpha=ALPHA
        )
...
def test_calculate_welch_ttest_constant_groups_identical(
    service: StatsService, group_constant_G: list[float]
):
    ...
    t_stat, p_val, df, ci = service._calculate_welch_ttest(
        const_group1, const_group2
    )
    ...
    assert (
        np.isnan(t_stat) or t_stat == 0.0
    )  # Scipy behavior can vary slightly with versions/inputs
    assert np.isnan(p_val) or p_val == 1.0
    # CI might also be (0,0) or (nan,nan) or based on diff_mean (0)
    assert (np.isnan(ci[0]) and np.isnan(ci[1])) or (
        ci[0] == 0.0 and ci[1] == 0.0
    )


def test_calculate_welch_ttest_constant_groups_different(
    service: StatsService,
):
    ...
    assert np.isinf(
        t_stat
    )  # t will be -inf because group_a mean < group_b mean
...
def test_perform_t_test_analysis_constant_groups(service: StatsService):
    ...
    result_identical = service.perform_t_test_analysis(
        const_group1, const_group2
    )
    ...
    assert (
        np.isnan(result_identical.statistic)
        or result_identical.statistic == 0.0
    )
    assert (
        np.isnan(result_identical.p_value) or result_identical.p_value == 1.0
    )
    assert not result_identical.is_significant_05  # If p_value is 1.0 or NaN

    const_group3 = [8.0] * 10  # Different
    result_different = service.perform_t_test_analysis(
        const_group1, const_group3
    )
...
def test_welch_degrees_of_freedom_calculation(service: StatsService):
    ...
    scipy_t, scipy_p = scipy_stats.ttest_ind(
        group_r_a, group_r_b, equal_var=False
    )
    ...
    assert result.degrees_freedom == pytest.approx(
        8.9996, abs=1e-2
    )  # Check if our df calculation is close
    ...
    expected_ci = scipy_stats.t.interval(
        0.95,
        df=result.degrees_freedom,
        loc=mean_diff_calc,
        scale=std_err_diff_calc,
    )

    assert result.confidence_interval_95[0] == pytest.approx(
        expected_ci[0], abs=1e-4
    )
    assert result.confidence_interval_95[1] == pytest.approx(
        expected_ci[1], abs=1e-4
    )
    # And compare roughly to R's output
    assert result.confidence_interval_95[0] == pytest.approx(
        -4.051, abs=5e-2
    )  # Looser approx due to potential df differences
...
def test_one_group_zero_variance(
    service: StatsService, group_normal_A: list[float]
):
    const_group = [5.0] * 10
    ...
    )  # Shapiro on const data is p=1

    # Compare with scipy
    scipy_t, scipy_p = scipy_stats.ttest_ind(
        const_group, group_normal_A, equal_var=False
    )
    ...
    scipy_t_rev, scipy_p_rev = scipy_stats.ttest_ind(
        group_normal_A, const_group, equal_var=False
    )
