...
class TTestResultModel(Base):
    ...
    experiment_id: Mapped[uuid_pkg.UUID] = mapped_column(ForeignKey("experiments.id"), unique=True) # One-to-one
    ...
    confidence_interval_95_lower: Mapped[float] = mapped_column(Float, nullable=False)
    confidence_interval_95_upper: Mapped[float] = mapped_column(Float, nullable=False)

    is_significant_05: Mapped[bool] = mapped_column(sa.Boolean, nullable=False)

    normality_p_value_group_a: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    normality_p_value_group_b: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
...
class ExperimentModel(Base):
    ...
    mlflow_run_id: Mapped[Optional[str]] = mapped_column(String(32), nullable=True, index=True) # MLflow run IDs are typically 32 chars
    ...
    tracking_warnings: Mapped[Optional[List[str]]] = mapped_column(JSONB, nullable=True)
