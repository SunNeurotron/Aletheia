...
from sqlalchemy.orm import (  # Removed joinedload as it's not used here directly now
    Session,
)

# Importar entidades de dominio y modelos de infraestructura
from ..domain.entities import Experiment as DomainExperiment
from ..domain.entities import TTestResult as DomainTTestResult
...
class SQLAlchemyStatsRepository:  # Podría heredar de un StatsRepositoryPort si se define una interfaz abstracta
    ...
    def _to_domain_ttest_result(
        self, result_model: Optional[TTestResultModel]
    ) -> Optional[DomainTTestResult]:  # Added Optional type hint
        if not result_model:
            return None
        return DomainTTestResult(
            ...
            confidence_interval_95=[
                result_model.confidence_interval_95_lower,
                result_model.confidence_interval_95_upper,
            ],
    ...
    def _to_domain_experiment(
        self, exp_model: ExperimentModel
    ) -> DomainExperiment:
        ...
        tracking_warnings_list = (
            exp_model.tracking_warnings
            if exp_model.tracking_warnings is not None
            else []
        )
    ...
    def save(self, experiment_domain: DomainExperiment) -> DomainExperiment:
        logger.debug(
            f"Attempting to save experiment ID: {experiment_domain.id}"
        )
        try:
            # Convertir entidad de dominio a modelo SQLAlchemy
            exp_model = (
                self.db.query(ExperimentModel)
                .filter(ExperimentModel.id == experiment_domain.id)
                .first()
            )
            if not exp_model:
            ...
            exp_model.tracking_warnings = (
                experiment_domain.tracking_warnings
            )  # Guardar warnings
            ...
            if experiment_domain.result:
                ...
                if not result_model:
                    result_model = TTestResultModel(
                        id=uuid.uuid4(), experiment_id=exp_model.id
                    )  # Ensure ID is set for new TTestResultModel

                result_model.statistic = experiment_domain.result.statistic
                result_model.p_value = experiment_domain.result.p_value
                result_model.degrees_freedom = (
                    experiment_domain.result.degrees_freedom
                )
                result_model.mean_group_a = (
                    experiment_domain.result.mean_group_a
                )
                result_model.variance_group_a = (
                    experiment_domain.result.variance_group_a
                )
                result_model.mean_group_b = (
                    experiment_domain.result.mean_group_b
                )
                result_model.variance_group_b = (
                    experiment_domain.result.variance_group_b
                )
                result_model.confidence_interval_95_lower = (
                    experiment_domain.result.confidence_interval_95[0]
                )
                result_model.confidence_interval_95_upper = (
                    experiment_domain.result.confidence_interval_95[1]
                )
                result_model.is_significant_05 = (
                    experiment_domain.result.is_significant_05
                )
                result_model.normality_p_value_group_a = (
                    experiment_domain.result.normality_p_value_group_a
                )
                result_model.normality_p_value_group_b = (
                    experiment_domain.result.normality_p_value_group_b
                )
            ...
            if (
                exp_model.result_model
            ):  # Refrescar también el resultado si existe
            ...
            self.db.rollback()
            logger.error(
                f"Database error while saving experiment ID {experiment_domain.id}: {e}",
                exc_info=True,
            )
    ...
    def get(self, experiment_id: UUID) -> Optional[DomainExperiment]:
        ...
        try:
            ...
            logger.error(
                f"Database error while retrieving experiment ID {experiment_id}: {e}",
                exc_info=True,
            )
            raise
    ...
    ) -> Tuple[List[DomainExperiment], int]:
        logger.debug(f"Listing experiments with skip={skip}, limit={limit}")
        try:
            total_count = (
                self.db.query(func.count(ExperimentModel.id)).scalar() or 0
            )  # Ensure 0 if None

            query = self.db.query(ExperimentModel).order_by(
            ...
            domain_experiments = [
                self._to_domain_experiment(em) for em in exp_models
            ]
            logger.info(
                f"Retrieved {len(domain_experiments)} experiments. Total count: {total_count}"
            )
            return domain_experiments, total_count
        except SQLAlchemyError as e:
            logger.error(
                f"Database error while listing experiments: {e}", exc_info=True
            )
            raise
