...
class StatsService:
    ...
    def __init__(self, random_state: int = 42):
        ...
        np.random.seed(self.random_state) # For reproducibility if any numpy RNG is used
    ...
    def _calculate_welch_ttest(
        ...
        if len(group_a) < 2 or len(group_b) < 2:
            raise ValueError("Each group must contain at least two observations for t-test.")

        # Perform Welch's t-test (assumes unequal variances)
        t_statistic, p_value = stats.ttest_ind(group_a, group_b, equal_var=False, nan_policy='raise')
        ...
        if s1_sq == 0 and s2_sq == 0: # Both groups have zero variance
             degrees_freedom = float(n1 + n2 - 2) # Or handle as a special case / error
            ...
            if n1 <2 or n2<2 : degrees_freedom = 1.0 # avoid df < 1
        ...
        if std_err_diff == 0: # Avoid division by zero if both groups are constant and identical
    ...
    def perform_t_test_analysis(
        ...
        if len(group_a) < 3 or len(group_b) < 3:
            ...
            raise ValueError("Each group must contain at least three observations for normality testing and t-test.")
        ...
        t_statistic, p_value, df, ci = self._calculate_welch_ttest(group_a, group_b, alpha)
...
if __name__ == "__main__":
    ...
    group2_normal = [3.0, 3.2, 3.5, 3.8, 4.0, 4.2, 4.5, 4.8, 5.0, 5.5] # n=10, different mean
    ...
    try:
        ...
        print(f"Confidence Interval (95%): ({result1.confidence_interval_95[0]:.3f}, {result1.confidence_interval_95[1]:.3f})")
        print(f"Significant at alpha=0.05: {result1.is_significant_05}")
        print(f"Normality Group A (p-value): {result1.normality_p_value_group_a:.3f}")
        print(f"Normality Group B (p-value): {result1.normality_p_value_group_b:.3f}")
    ...
    try:
        result2 = service.perform_t_test_analysis(group1_normal, group3_non_normal)
        print(f"Comment: {result2.comment}")
        print(f"P-Value: {result2.p_value:.3f}")
        print(f"Normality Group A (p-value): {result2.normality_p_value_group_a:.3f}")
        print(f"Normality Group B (p-value): {result2.normality_p_value_group_b:.3f}")
    except ValueError as e:
        print(f"Error: {e}")
    ...
    try:
        result_const = service.perform_t_test_analysis(group_const1, group_const2)
        print(f"T-Statistic: {result_const.statistic}") # Should be 0 or nan by scipy
        print(f"P-Value: {result_const.p_value}") # Should be 1.0 or nan by scipy
        print(f"Confidence Interval (95%): {result_const.confidence_interval_95}")
    ...
    try:
        result_const_normal = service.perform_t_test_analysis(group_const1, group1_normal)
        print(f"T-Statistic: {result_const_normal.statistic:.3f}")
        print(f"P-Value: {result_const_normal.p_value:.3f}")
        print(f"Confidence Interval (95%): ({result_const_normal.confidence_interval_95[0]:.3f}, {result_const_normal.confidence_interval_95[1]:.3f})")
