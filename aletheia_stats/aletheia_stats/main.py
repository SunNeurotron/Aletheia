...
from .infrastructure.database import SQLALCHEMY_DATABASE_URL as STATS_DB_URL
from .infrastructure.database import SessionLocal as StatsSessionLocal
from .infrastructure.database import engine as stats_db_engine
...
from .presentation.api import (
    MLFLOW_TRACKING_URI as STATS_MLFLOW_URI,  # Usar la URI configurada en api.py
)
...
APP_LOG_LEVEL = os.getenv(
    "APP_LOG_LEVEL", "INFO"
).upper()  # Variable de entorno específica para la app
logging.basicConfig(
    level=APP_LOG_LEVEL,
...
logger.info(
    f"Aletheia-Stats API starting with application log level: {APP_LOG_LEVEL}"
)
...
if "*" in origins and len(origins) > 1:
    logger.warning(
        "CORS_ALLOWED_ORIGINS contains '*' along with other origins. This is likely a misconfiguration. Defaulting to '*' for broad compatibility during dev."
    )
    origins = ["*"]
...
async def startup_event():
    ...
    try:
        ...
        if stats_db_engine:
            logger.info(
                f"Database URL configured: {STATS_DB_URL[:STATS_DB_URL.find('@') if '@' in STATS_DB_URL else len(STATS_DB_URL)]}"
            )
            ...
            logger.critical(
                "Database engine (stats_db_engine) is not available. Database operations will fail."
            )
        ...
        if STATS_MLFLOW_URI and STATS_MLFLOW_URI.lower() != "none":
            ...
            try:
                ...
                logger.info(
                    "MLflow client configured successfully at startup (tracker instance created)."
                )
            except Exception as mlflow_err:
                logger.error(
                    f"Failed to initialize MLflow client with URI '{STATS_MLFLOW_URI}': {mlflow_err}",
                    exc_info=True,
                )
        else:
            logger.warning(
                "MLflow Tracking URI is not configured or set to 'none'. MLflow integration will be disabled."
            )

        logger.info("Aletheia-Stats API startup sequence complete.")
    except (
        ConnectionError
    ) as ce:  # Specific error for DB connection issues from get_db_session_stats
        logger.critical(
            f"Database connection/initialization error during startup: {ce}",
            exc_info=True,
        )
        ...
        logger.critical(
            f"An unexpected error occurred during API startup: {e}",
            exc_info=True,
        )
...
app.include_router(
    stats_api_router, prefix="/api/v1"
)  # Removed tags here as they are in the router itself
logger.info("Stats API router (stats_api_router) included at prefix /api/v1.")


# --- Root Endpoint (Optional) ---
@app.get(
    "/", summary="Aletheia-Stats API Root", tags=["General"]
)  # Clarified summary
async def read_root():
    """
...
if __name__ == "__main__":
    ...
    )  # Specific env var for stats module
    API_PORT = int(
        os.getenv("STATS_API_PORT", "8001")
    )  # Default to 8001 to avoid main Aletheia API
    LOG_LEVEL_UVICORN = os.getenv(
        "STATS_LOG_LEVEL_UVICORN", "info"
    ).lower()  # Specific env var

    logger.info(
        f"Attempting to start Uvicorn server for Aletheia-Stats on {API_HOST}:{API_PORT} with Uvicorn log level {LOG_LEVEL_UVICORN}"
    )
