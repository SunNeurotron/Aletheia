...
class MLflowExperimentTracker:
    ...
    def __init__(self, tracking_uri: str):
        ...
        logger.info(f"MLflowExperimentTracker initialized with tracking URI: {self.tracking_uri}")

    def start_run(self, experiment_name: str, run_name: Optional[str] = None, tags: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        try:
            ...
            if experiment is None:
                logger.info(f"Experiment '{experiment_name}' not found. Creating new experiment.")
            ...
            if existing_run_id:
                logger.info(f"Attempting to resume existing MLflow run: {existing_run_id}")
                ...
                active_run = mlflow.start_run(run_id=existing_run_id, experiment_id=experiment_id, run_name=run_name, nested=True)

            else:
                logger.info(f"Starting a new MLflow run for experiment ID: {experiment_id}")
                active_run = mlflow.start_run(experiment_id=experiment_id, run_name=run_name, nested=True)

            self._active_run_id = active_run.info.run_id
            logger.info(f"MLflow run started successfully. Run ID: {self._active_run_id}")
            ...
            logger.error(f"Failed to start MLflow run for experiment '{experiment_name}': {e}", exc_info=True)
            raise
    ...
    def end_run(self, status: str = "FINISHED") -> None:
        ...
        if self._active_run_id and mlflow.active_run():
            ...
            if current_run_id == self._active_run_id:
                ...
                logger.info(f"MLflow run {self._active_run_id} ended with status: {status}.")
            else:
                # This case should ideally not happen if start_run and end_run are managed well.
                logger.warning(f"Attempted to end run {self._active_run_id}, but active run is {current_run_id}. Ending current active run.")
                mlflow.end_run(status=status)
            self._active_run_id = None
        elif self._active_run_id:
             logger.warning(f"No active MLflow run found by mlflow.active_run(), but tracker has active_run_id {self._active_run_id}. Cannot end run explicitly by status. It might have been ended elsewhere or implicitly.")
    ...
    def log_param(self, key: str, value: Any) -> None:
        ...
        if not self._active_run_id or not mlflow.active_run():
            logger.warning("No active MLflow run to log parameter. Start a run first.")
            return
        try:
            mlflow.log_param(key, value)
            logger.debug(f"Logged param '{key}': {value} to run {self.active_run_id}")
        except Exception as e:
            logger.error(f"Failed to log param '{key}' to MLflow run {self.active_run_id}: {e}", exc_info=True)


    def log_params(self, params: Dict[str, Any]) -> None:
        """Logs multiple parameters for the active MLflow run."""
        if not self._active_run_id or not mlflow.active_run():
            logger.warning("No active MLflow run to log parameters. Start a run first.")
            return
        try:
            mlflow.log_params(params)
            logger.debug(f"Logged params: {params} to run {self.active_run_id}")
        except Exception as e:
            logger.error(f"Failed to log params to MLflow run {self.active_run_id}: {e}", exc_info=True)

    def log_metric(self, key: str, value: float, step: Optional[int] = None) -> None:
        """Logs a single metric for the active MLflow run."""
        if not self._active_run_id or not mlflow.active_run():
            logger.warning("No active MLflow run to log metric. Start a run first.")
            return
        try:
            mlflow.log_metric(key, value, step=step)
            logger.debug(f"Logged metric '{key}': {value} (step: {step}) to run {self.active_run_id}")
        except Exception as e:
            logger.error(f"Failed to log metric '{key}' to MLflow run {self.active_run_id}: {e}", exc_info=True)

    def log_metrics(self, metrics: Dict[str, float], step: Optional[int] = None) -> None:
        """Logs multiple metrics for the active MLflow run."""
        if not self._active_run_id or not mlflow.active_run():
            logger.warning("No active MLflow run to log metrics. Start a run first.")
            return
        try:
            mlflow.log_metrics(metrics, step=step)
            logger.debug(f"Logged metrics: {metrics} (step: {step}) to run {self.active_run_id}")
        except Exception as e:
            logger.error(f"Failed to log metrics to MLflow run {self.active_run_id}: {e}", exc_info=True)


    def set_tag(self, key: str, value: Any) -> None:
        """Sets a tag for the active MLflow run."""
        if not self._active_run_id or not mlflow.active_run():
            logger.warning("No active MLflow run to set tag. Start a run first.")
            return
        try:
            mlflow.set_tag(key, value)
            logger.debug(f"Set tag '{key}': {value} for run {self.active_run_id}")
        except Exception as e:
            logger.error(f"Failed to set tag '{key}' for MLflow run {self.active_run_id}: {e}", exc_info=True)

    def set_tags(self, tags: Dict[str, Any]) -> None:
        """Sets multiple tags for the active MLflow run."""
        if not self._active_run_id or not mlflow.active_run():
            logger.warning("No active MLflow run to set tags. Start a run first.")
            return
        try:
            mlflow.set_tags(tags)
            logger.debug(f"Set tags: {tags} for run {self.active_run_id}")
        except Exception as e:
            logger.error(f"Failed to set tags for MLflow run {self.active_run_id}: {e}", exc_info=True)
    ...
    def active_run_id(self) -> Optional[str]:
        ...
        if mlflow.active_run():
            ...
            if self._active_run_id != current_mlflow_run_id:
                logger.warning(f"Internal active_run_id '{self._active_run_id}' differs from mlflow.active_run() '{current_mlflow_run_id}'. Updating to current.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        run_id = tracker.start_run(experiment_name=exp_name, run_name="TestRun_123", tags={"user": "test_user", "version": "0.1"})
        ...
        run_id_2 = tracker.start_run(experiment_name=exp_name, run_name="TestRun_456")
        ...
        logger.error(f"An error occurred during MLflowExperimentTracker test: {e}", exc_info=True)
        logger.error("Please ensure an MLflow tracking server is running at the specified URI if using http.")
        logger.error("Alternatively, for local file-based tracking, use a URI like 'file:./mlruns_test'")
